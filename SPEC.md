...
- 배포: `@space/api` (core) + `@space/api-ui` (정적 번들, core가 내부적으로 서빙)
...
3. **Headless core + static UI** — core는 JSON spec + express static만 책임지고, UI는 번들된 정적 파일(`@space/api-ui/dist`)을 서빙. UI 교체/커스텀 가능.
...
├── package.json
├── packages/
│   ├── api/                  # @space/api  (core, NestJS 플러그인)
│   │   ├── src/
│   │   │   ├── index.ts
│   │   │   ├── decorators/
...
│   │   ├── package.json
│   │   └── tsup.config.ts
│   ├── api-ui/               # @space/api-ui  (React SPA, 정적 번들)
│   │   ├── src/
│   │   │   ├── main.tsx
│   │   │   ├── App.tsx
│   │   │   ├── styles/
...
export class SpaceApiModule {
  static setup(path: string, app: INestApplication, options: SpaceApiOptions): void {
    ...
    // UI (static from @space/api-ui/dist)
...
export function serveUi(httpAdapter: HttpAdapterHost['httpAdapter'], basePath: string) {
  const uiDir = require.resolve('@space/api-ui/dist/index.html').replace('/index.html', '');
...
- `package.json`:
  ```json
  {
    ...
    "dependencies": {
      "@space/api-ui": "workspace:*"
    }
  }
  ```
- `@space/api-ui`는 **dependency**로 들고, publish 시점에 `workspace:*` → 실제 버전으로 치환(pnpm이 자동 처리).
...
pnpm install
pnpm -r build                         # 전체 빌드
pnpm --filter @space/api-ui dev       # UI 개발 (Vite dev + 목 OpenAPI)
